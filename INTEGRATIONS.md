...
**setup-scripts** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `setup-scripts` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
setup-scripts ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
setup-scripts ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
setup-scripts ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `setup-scripts` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="setup-scripts") for rec in records]   # records = your JSON output
